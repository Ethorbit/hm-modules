...
{
    options = with lib; {
        ethorbit.home-manager.polybar.scripts.weather = lib.mkOption {
            type = types.package;
            default = (pkgs.writeShellScript "weather.sh" ''
                host="${pkgs.toybox}/bin/host"
                grep="${pkgs.gash-utils}/bin/grep"
                awk="${pkgs.gawk}/bin/awk"
                head="${pkgs.gash-utils}/bin/head"
                
                [ ! $(command -v weather-Cli) ] && echo "" && exit
